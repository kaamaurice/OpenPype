...
def create_global_app():
    ...
    # save a reference to the C++ window in a global var, to prevent the parent
    # being garbage collected for some reason this works here, but not in the
    # blender_applications init as a class attribute (self), and saving it in a
    # global in blender_applications.py causes blender to crash on startup.
...
def register():
    ...
    # hacky way to check if we already are waiting on bqt setup,
    # or bqt is already setup.
...
def unregister():
    ...
    """
    # todo, as long as blender is wrapped in qt,
    # unregistering operator & callback will cause issues,
