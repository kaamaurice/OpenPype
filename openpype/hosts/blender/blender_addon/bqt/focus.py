"""
when alt tabbing in and out of blender, a bug happens when blender is wrapped
in qt. some keys stay stuck, e.g. alt or control, resulting in the user
...
class QFocusOperator(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        """
        simulate key release to prevent blender from ignoring mouse input,
        after refocusing blender
        """
    ...
    def _detect_keyboard():
        ...
        for name, code in keycodes:
            ...
            # todo this bug fix is not perfect yet, blender works better
            # without this atm
            # # if the first key pressed is one of the following,
            # # don't simulate a key release, since it causes this bug:
            # # the first keypress on re-focus blender will be ignored,
            # # e.g. ctrl + v will just be v
        ...
        # todo, fix: blender occasionally still frozen input, despite having
        # run the above code but when we click the mouse,
