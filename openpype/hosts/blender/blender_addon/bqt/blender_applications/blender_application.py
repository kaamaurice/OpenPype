...
class BlenderApplication(QApplication):
    ...
    def _get_application_icon() -> QIcon:
        """
        This finds the running blender process, extracts the blender icon from
        the blender.exe file on disk and saves it to the user's temp folder.
        ...
        icon_filepath = (
            Path(__file__).parents[1] / "images" / "blender_icon_16.png"
        )
        icon = QIcon()
    ...
    def _set_window_geometry(self):
        """
        Loads stored window geometry preferences and applies them to the
        QWindow.setGeometry() sets the size of the window minus the window
        frame.
        ...
        if settings.value(
            self._settings_key_full_screen, "false"
        ).lower() == "true":
            self.blender_widget.showFullScreen()
            return

        if settings.value(
            self._settings_key_maximized, "false"
        ).lower() == "true":
    ...
    def notify(self, receiver: QObject, event: QEvent) -> bool:
        ...
        if (
            ...
            and receiver in (self.blender_widget, self._blender_window)
        ):
            # catch the close event when clicking close on the qt window,
            # ignore the event, and ask user if they want to close blender
            # if unsaved changes.
            # if this is successful, blender will trigger bqt.on_exit()
            event.ignore()

            if os.getenv("BQT_DISABLE_CLOSE_DIALOGUE") == "1":
                # this triggers the default blender close event,
                # showing thesave dialog if needed
    ...
    def store_window_geometry(self):
        ...
        The .geometry() method on QWindow includes the size of the application
        minus the window frame.
        ...
        settings.beginGroup(self._settings_key_window_group_name)
        settings.setValue(
            self._settings_key_geometry, self.blender_widget.geometry()
        )
        settings.setValue(
            self._settings_key_maximized, self.blender_widget.isMaximized()
        )
        settings.setValue(
            self._settings_key_full_screen, self.blender_widget.isFullScreen()
        )
        settings.endGroup()
